...
namespace UniversalCalculator.Forms
{
    public class MainForm : Form
    {
        ...
        public MainForm()
        {
            _calculator = new CalculatorControl(CalculatorMode.PNumber);
            _calculator.DisplayUpdated += (text) => UpdateDisplay();
            ...
            _calculator.ModeChanged += (m) => UpdateModeUI();
            SetupKeyMapping();
        ...
        private void InitializeComponent()
        {
            ...
            {
                Location = new Point(65, 98),
                Size = new Size(120, 20),
                DropDownStyle = ComboBoxStyle.DropDownList,
                Font = new Font("Segoe UI", 9)
            };
            modeComboBox.Items.AddRange(new object[] { "P-ичные числа", "Простые дроби", "Комплексные числа" });
        ...
        private void CreateButtons()
        {
            ...
            // Ряд 2 — Очистка и функции
            AddDigitButton("0", 1, 0, 0);
            AddButton("CE", 1, 1, CalculatorCommand.ClearEntry);
            AddButton("C", 1, 2, CalculatorCommand.Clear);
            AddButton("x²", 1, 3, CalculatorCommand.Square);
            AddButton("1/x", 1, 4, CalculatorCommand.Inverse);

            // Ряд 3
            AddDigitButton("1", 2, 0, 1);
            AddDigitButton("2", 2, 1, 2);
            AddDigitButton("3", 2, 2, 3);
            ...
            // Ряд 5
            AddDigitButton("7", 4, 0, 7);
            AddDigitButton("8", 4, 1, 8);
            AddDigitButton("9", 4, 2, 9);
            ...
            AddButton("i", 5, 3, CalculatorCommand.ImaginaryUnit);
        ...
        private void AddDigitButton(string text, int row, int col, int digitValue)
        {
            ...
            btn.Click += (s, e) =>
            {
                ...
                switch (digit)
                {
                    case 10: cmd = CalculatorCommand.DigitA; break;
                    case 11: cmd = CalculatorCommand.DigitB; break;
                    case 12: cmd = CalculatorCommand.DigitC; break;
                    case 13: cmd = CalculatorCommand.DigitD; break;
                    case 14: cmd = CalculatorCommand.DigitE; break;
                    case 15: cmd = CalculatorCommand.DigitF; break;
                    default: cmd = (CalculatorCommand)((int)CalculatorCommand.Digit0 + digit); break;
                }
        ...
        private void SetupKeyMapping()
        {
            _keyMapping = new Dictionary<Keys, CalculatorCommand>
            {
                ...
                { Keys.I, CalculatorCommand.ImaginaryUnit } 
            };
        }
        ...
        private void Form_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            {
                // Проверка допустимости цифры для p-ичных чисел
                if (e.KeyCode >= Keys.D0 && e.KeyCode <= Keys.D9)
                {
                    int digit = e.KeyCode - Keys.D0;
                    if (_calculator.Mode == CalculatorMode.PNumber && digit >= _calculator.CurrentBase)
                    {
        ...
        {
            CalculatorMode mode = modeComboBox.SelectedIndex switch
            {
                0 => CalculatorMode.PNumber,
                1 => CalculatorMode.Fraction,
                2 => CalculatorMode.Complex,
                _ => CalculatorMode.PNumber
            };
            _calculator.ChangeMode(mode);
        }

        private void UpdateDigitButtonsState()
        {
            int currentBase = _calculator.CurrentBase;
            bool isPNumberMode = _calculator.Mode == CalculatorMode.PNumber;

            foreach (var btn in _digitButtons)
            {
                if (btn.Tag is int digit)
                {
                    bool isEnabled = !isPNumberMode || digit < currentBase;
            ...
            foreach (var btn in _hexButtons)
            {
                if (btn.Tag is int digit)
                {
                    ...
                    btn.Enabled = isEnabled;
                    btn.BackColor = isEnabled ? Color.LightSteelBlue : Color.LightGray;
                    btn.Visible = isPNumberMode;
                }
            }

            baseComboBox.Visible = isPNumberMode;
        }
        ...
        {
            displayTextBox.Text = _calculator.DisplayText;
            expressionLabel.Text = _calculator.GetCurrentExpression();
        }
    }
}