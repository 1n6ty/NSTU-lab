...
void Server::run() {
    ...
    while (true) {
        ...
        SOCKET clientSocket = accept(serverSocket, (struct sockaddr*)&clientSIN, &clientSINSize);
        ...
        for (size_t i = 0; i < text.length(); ++i) {
            if (i > 0 && islower((unsigned char)text[i - 1]) && isupper((unsigned char)text[i])) {
                result += ' ';
            }
            result += text[i];
        }
        ...
        res = send(clientSocket, result.c_str(), result.length(), 0);
